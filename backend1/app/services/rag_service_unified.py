...
class RAGServiceUnified:
    ...
    def generate_comprehensive_answer(self, 
        ...
        """Tạo câu trả lời toàn diện từ search results"""
        try:
            ...
            for i, result in enumerate(top_results, 1):
                ...
                if important_sentences:
                    ...
                    # Nếu không tìm thấy câu cụ thể, lấy đoạn đầu
                    answer_parts.append(f"**{i}. Theo {display_name}:**")
    ...
    def _extract_important_sentences(self, content: str, question_keywords: str) -> List[str]:
        """Trích xuất câu quan trọng từ content"""
        sentences = content.replace('\n', ' ').split('.')
        important_sentences = []
        
        keywords = ['an toàn thông tin', 'information security', 'bảo mật', 'định nghĩa', 
                   'quy định', 'tiêu chuẩn', 'bảo vệ', 'security', 'confidentiality', 
                   'integrity', 'availability']
        
        for sentence in sentences:
            sentence = sentence.strip()
            if len(sentence) > 30:  # Bỏ qua câu quá ngắn
                sentence_lower = sentence.lower()
                
                # Kiểm tra có chứa keywords quan trọng
                if any(keyword in sentence_lower for keyword in keywords):
                    important_sentences.append(sentence)
                
                # Nếu đã có đủ 3 câu thì dừng
